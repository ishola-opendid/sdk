...
import { makeRecipientKeypair, importRecipientPublicKey, importRecipientPrivateKey } from "./keys.js";
...
class CoreAPI {
  ...
  async verifyClaim(issuerId: string, claimId: string): Promise<did.DIDDocument> {
    await this.ensureInitialized();
  ...
  async bulkVerifyClaims(issuerId: string, claimIds: string[]): Promise<did.DIDDocument[]> {
  ...
  private async encryptClaim(didDoc: did.DIDDocument): Promise<string> {
    ...
    try {
      const encrypted = await encryptDidDoc(didDoc, this.recipientKeyPair.pubJwk);
      return encrypted;
    } catch (error) {
      throw new Error(`Failed to encrypt claim: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async storeOnChain(issuerId: string, subjectId: string, cid: string): Promise<void> {
    ...
    try {
      ...
      await contract.methods.storeClaim(
        issuerId,
        subjectId,
        cid,
        timestamp
      ).send({
        from: await this.web3Provider.eth.getAccounts().then((accounts: string[]) => accounts[0])
      });

    } catch (error) {
      throw new Error(`Failed to store claim on chain: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async getClaimFromChain(issuerId: string, claimId: string): Promise<ChainClaimData> {
    if (!this.web3Provider || !this.contractAddress) {
      throw new Error("Web3 provider and contract address must be configured");
    }
    ...
    try {
      ...
      throw new Error(`Failed to retrieve claim from chain: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async decryptClaim(encrypted: string): Promise<did.DIDDocument> {
    ...
    try {
      // Decrypt the claim
      const decryptedData = await decryptDidDoc(encrypted, this.recipientKeyPair.privJwk);
      ...
      throw new Error(`Failed to decrypt claim: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private validateClaimIntegrity(
    ...
    if (didDoc.issuerId !== expectedIssuerId) {
      throw new Error(`Claim integrity violation: issuer ID mismatch. Expected: ${expectedIssuerId}, Got: ${didDoc.issuerId}`);
    }

    // Validate the subjectId matches chain data
    if (didDoc.subjectId !== chainData.subjectId) {
      throw new Error(`Claim integrity violation: subject ID mismatch. Expected: ${chainData.subjectId}, Got: ${didDoc.subjectId}`);
    }
